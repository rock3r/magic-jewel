...
case_group_cases() {
  case "$1" in
    ...
    shader-descriptor-invalid)
      ...
      ;;
    gradient-invalid)
      echo "commands-invalid-linear-gradient-stroke-width-fallback"
      ;;
...
if [[ -z "${CASES_WAS_SET}" && -z "${CASE_GROUPS}" ]]; then
  ...
  CASES="${CASES/commands-gradient-stroke commands-image-filter/commands-gradient-stroke commands-invalid-linear-gradient-stroke-width-fallback commands-image-filter}"
