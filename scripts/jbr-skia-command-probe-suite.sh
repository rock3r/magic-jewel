...
run_named_case() {
  case "$1" in
    ...
    commands-blend-mode|commands-blend-mode-fallback)
      ...
      ;;
    commands-graphics-layer-fallback)
      run_case "$1" \
        MAGIC_JEWEL_COMPOSE_GRAPHICS_LAYER=true \
        EXPECT_COMMAND_FALLBACK=true \
        EXPECT_COMMAND_FALLBACK_REASON=graphicsLayer
      ;;
