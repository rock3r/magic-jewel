...
`commands-turbulence-shader-fallback` enables `MAGIC_JEWEL_COMPOSE_TURBULENCE_SHADER=true` and asserts Skia turbulence shaders stay on structured `shader` fallback until JBR owns an explicit descriptor for that Perlin variant.
