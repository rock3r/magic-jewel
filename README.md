...
`commands-raw-linear-gradient-shader-fallback`, `commands-raw-noise-shader-fallback`, and `commands-raw-turbulence-shader-fallback` keep raw Skia gradient and Perlin/noise shader objects on structured `shader` fallback, proving descriptor paths do not reopen the raw pointer boundary.
