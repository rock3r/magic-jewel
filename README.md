...
Stable descriptor rows also assert reuse with max-count gates: tint/color-matrix/lighting/image color-filter rows expect a single effect-handle define, `commands-runtime-effect-pure-color` expects a single shader-handle define, `commands-runtime-effect-child-only` expects exactly the child plus RuntimeEffect shader handles with cache hits, `commands-linear-gradient-shader-color-filter` expects exactly the wrapped shader/color-filter descriptor handles, and `commands-composite-shader` expects exactly the three shader handles for its dst/src/composite tree. RuntimeEffect descriptor rows additionally assert JBR-side handle use markers, proving the handles were consumed by replay and not only defined.
