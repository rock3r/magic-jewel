...
The parity suite runs deterministic old/new window captures for the rich baseline scene plus focused native-text, RuntimeEffect, and graphics-layer effect scenes. Use `CASES="parity-rich parity-runtime-effect-pure-color"` to run a subset. It writes `suite.tsv` with one row per case, including whole-window and Compose-canvas bad-pixel ratios plus the report and diff image paths.
