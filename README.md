...
`parity-native-text` enables `JBR_SKIA_NATIVE_TEXT=true`, requires paragraph text commands, disables the image-ref expectation, and uses a text-aware whole-window threshold because native text is expected to differ slightly from the default text-as-image fidelity path. The command probe's `commands-native-text` row also requires simple text commands so the ABI 101 font-family/style metadata path stays covered by the live suite.
